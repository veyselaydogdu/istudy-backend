...
import { FoodIngredient, Meal, School, Allergen } from '@/types';
import { Plus, Trash2, Edit2, X, Utensils, Leaf, ShieldAlert } from 'lucide-react';

type Tab = 'meals' | 'ingredients' | 'allergens';

const MEAL_TYPE_LABELS: Record<string, string> = {
    breakfast: 'Kahvaltı', lunch: 'Öğle Yemeği', snack: 'Ara Öğün', dinner: 'Akşam Yemeği',
};
...
export default function MealsPage() {
    ...
    const [mealForm, setMealForm] = useState({ name: '', meal_type: 'lunch', ingredient_ids: [] as number[] });
    ...
    useEffect(() => { if (activeTab === 'meals' && selectedSchoolId) fetchMeals(); }, [activeTab, selectedSchoolId, fetchMeals]);
    useEffect(() => { if (activeTab === 'ingredients') fetchIngredients(); }, [activeTab, fetchIngredients]);

    // ── Yemek CRUD ──────────────────────────────────────────────
    const openCreateMeal = () => {
        setEditingMeal(null);
        setMealForm({ name: '', meal_type: 'lunch', ingredient_ids: [] });
    ...
    const handleIngredientSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingIngredient) {
                await apiClient.put(`/food-ingredients/${editingIngredient.id}`, ingredientForm);
                toast.success('Besin öğesi güncellendi.');
            } else {
                await apiClient.post('/food-ingredients', ingredientForm);
    ...
    const handleDeleteIngredient = async (ing: FoodIngredient) => {
        ...
        const result = await Swal.fire({
            title: 'Besin Öğesi Sil', text: `"${ing.name}" silinecek.`, icon: 'warning',
            showCancelButton: true, confirmButtonText: 'Evet, Sil', cancelButtonText: 'İptal', confirmButtonColor: '#e7515a',
        });
        if (!result.isConfirmed) return;
        try {
    ...
    return (
        <div className="p-6">
            ...
            <div className="panel">
                    ...
                    <>
                        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <select className="form-select mt-1" value={selectedSchoolId} onChange={e => setSelectedSchoolId(e.target.value)}>
                                    {schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                                </select>
                            </div>
                            <button type="button" className="btn btn-primary btn-sm gap-2 self-end" onClick={openCreateMeal} disabled={!selectedSchoolId}>
                                    ...
                                    <div key={meal.id} className="rounded border border-[#ebedf2] p-4 dark:border-[#1b2e4b]">
                                        <div className="mb-2 flex items-start justify-between">
                                            <div>
                                                ...
                                                <span className="text-xs text-[#888ea8]">{MEAL_TYPE_LABELS[meal.meal_type ?? ''] ?? meal.meal_type}</span>
                                            </div>
                                        ...
                                        {meal.ingredients && meal.ingredients.length > 0 && (
                                            <div className="flex flex-wrap gap-1">
                                                {meal.ingredients.map(i => (
                                                    <span key={i.id} className="rounded-full bg-primary/10 px-2 py-0.5 text-xs text-primary">{i.name}</span>
                                                ))}
                                            </div>
                                        )}
                                    </div>
                            ...
                            <div className="table-responsive">
                                <table className="table-hover">
                                    ...
                                    <tbody>
                                        {ingredients.map(ing => (
                                            <tr key={ing.id}>
                                                <td className="font-medium text-dark dark:text-white">{ing.name}</td>
                                                <td>
                                                    <div className="flex flex-wrap gap-1">
                ...
                {activeTab === 'allergens' && (
                    <>
                        ...
                        {/* Tenant allerjenler */}
                        <div>
                            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-[#888ea8]">
                                Kuruma Özel Allerjenler
                            </h3>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-lg bg-white p-6 dark:bg-[#0e1726]">
                        <div className="mb-4 flex items-center justify-between">
                            ...
                            <button type="button" onClick={() => setShowMealModal(false)} className="text-[#888ea8] hover:text-danger"><X className="h-5 w-5" /></button>
                        </div>
                        <form onSubmit={handleMealSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-dark dark:text-white-light">Yemek Adı *</label>
                                <input type="text" className="form-input mt-1" value={mealForm.name} onChange={e => setMealForm(p => ({ ...p, name: e.target.value }))} required />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-dark dark:text-white-light">Öğün Türü</label>
                                <select className="form-select mt-1" value={mealForm.meal_type} onChange={e => setMealForm(p => ({ ...p, meal_type: e.target.value }))}>
                                    {Object.entries(MEAL_TYPE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="block mb-2 text-sm font-medium text-dark dark:text-white-light">Besin Öğeleri</label>
                                {ingredients.length === 0 && <p className="text-sm text-[#888ea8]">Besin öğesi yükleniyor...</p>}
                                <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto">
                                    {ingredients.map(ing => (
                                        <label key={ing.id} className="flex cursor-pointer items-center gap-2 rounded border border-[#ebedf2] p-2 hover:border-primary dark:border-[#1b2e4b]">
                                            <input type="checkbox" className="form-checkbox" checked={mealForm.ingredient_ids.includes(ing.id)} onChange={() => toggleIngredient(ing.id)} />
                                            <span className="text-sm text-dark dark:text-white">{ing.name}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
            ...
            {showIngredientModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-lg bg-white p-6 dark:bg-[#0e1726]">
                        <div className="mb-4 flex items-center justify-between">
                            <h2 className="text-lg font-bold text-dark dark:text-white">{editingIngredient ? 'Besin Öğesi Düzenle' : 'Yeni Besin Öğesi'}</h2>
                            <button type="button" onClick={() => setShowIngredientModal(false)} className="text-[#888ea8] hover:text-danger"><X className="h-5 w-5" /></button>
                        </div>
                        <form onSubmit={handleIngredientSubmit} className="space-y-4">
                            <div>
                                ...
                                <input type="text" className="form-input mt-1" value={ingredientForm.name} onChange={e => setIngredientForm(p => ({ ...p, name: e.target.value }))} required />
                            </div>

                            {allergens.length > 0 && (
                                <div>
                                    <label className="block mb-2 text-sm font-medium text-dark dark:text-white-light">Allerjenler</label>
                                    ...
                                    {globalAllergens.length > 0 && (
                                        <div className="mb-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto">
                                                {globalAllergens.map(a => (
                                                    <label key={a.id} className="flex cursor-pointer items-center gap-2 rounded border border-[#ebedf2] p-2 hover:border-primary dark:border-[#1b2e4b]">
                                                        <input type="checkbox" className="form-checkbox" checked={ingredientForm.allergen_ids.includes(a.id)} onChange={() => toggleAllergenInIngredient(a.id)} />
            ...
            {showAllergenModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-md rounded-lg bg-white p-6 dark:bg-[#0e1726]">
                        <div className="mb-4 flex items-center justify-between">
                            <h2 className="text-lg font-bold text-dark dark:text-white">{editingAllergen ? 'Allerjen Düzenle' : 'Yeni Allerjen'}</h2>
                            <button type="button" onClick={() => setShowAllergenModal(false)} className="text-[#888ea8] hover:text-danger"><X className="h-5 w-5" /></button>
                        </div>
                        <form onSubmit={handleAllergenSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-dark dark:text-white-light">Ad *</label>
                                <input type="text" className="form-input mt-1" value={allergenForm.name} onChange={e => setAllergenForm(p => ({ ...p, name: e.target.value }))} required />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-dark dark:text-white-light">Açıklama</label>
                                <textarea className="form-input mt-1" rows={2} value={allergenForm.description} onChange={e => setAllergenForm(p => ({ ...p, description: e.target.value }))} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-dark dark:text-white-light">Risk Seviyesi</label>
                                <select className="form-select mt-1" value={allergenForm.risk_level} onChange={e => setAllergenForm(p => ({ ...p, risk_level: e.target.value }))}>
