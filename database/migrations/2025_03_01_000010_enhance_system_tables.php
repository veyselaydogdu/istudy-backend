    ...
    */
    ...
    public function up(): void
    {
        ...
        Schema::create('school_enrollment_requests', function (Blueprint $table) {
            ...
            $table->foreignId('user_id')->constrained()->cascadeOnDelete()->comment('Talepte bulunan veli');
            $table->foreignId('family_profile_id')->nullable()->constrained()->cascadeOnDelete();
            $table->string('status')->default('pending')->comment('pending, approved, rejected');
            $table->text('message')->nullable()->comment('Velinin mesajı');
            $table->text('rejection_reason')->nullable()->comment('Red sebebi');
            $table->foreignId('reviewed_by')->nullable()->constrained('users')->cascadeOnDelete()->comment('Onaylayan/Reddeden yetkili');
        ...
        Schema::create('report_input_values', function (Blueprint $table) {
            $table->id();
            $table->foreignId('daily_child_report_id')->constrained('daily_child_reports')->cascadeOnDelete();
            $table->foreignId('report_template_input_id')->constrained('report_template_inputs')->cascadeOnDelete();
        ...
        Schema::table('daily_child_reports', function (Blueprint $table) {
            ...
            $table->foreignId('report_template_id')->nullable()->after('class_id')
                  ->constrained('report_templates')->cascadeOnDelete()
